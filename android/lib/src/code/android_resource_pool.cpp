 ...
 */
void rp::android_resource_pool::get_file
( const std::string& name, std::ostream& os )
{
  claw::logger << claw::log_verbose << "Getting asset " << name << std::endl;
 ...
 */
bool
rp::android_resource_pool::exists( const std::string& name ) const
{
  SDL_RWops* rw = SDL_RWFromFile( name.c_str(), "rb" );
